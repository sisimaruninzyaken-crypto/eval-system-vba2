...
Private Sub PositionTopRightButtons(f As MSForms.Frame)
    On Error Resume Next
    If Not btnLoadPrevCtl Is Nothing Then
        btnLoadPrevCtl.Top = 8
        btnLoadPrevCtl.Left = f.Width - btnLoadPrevCtl.Width - 140
    End If
...
Dim r As Long
ws.Activate
'r = PickRecentRowByID(ws.Cells(ActiveCell.row, HeaderCol("ID", ws)).value, 5, ws)


#If APP_DEBUG Then
    Debug.Print "ActiveSheet=", ActiveSheet.name, " ActiveRow=", ActiveCell.row, " ID=", ws.Cells(ActiveCell.row, HeaderCol("ID", ws)).value, " PickedRow=", r
#End If
...
Me.Controls("mpPhys").Pages(0).Controls("Frame8").Controls("mpROM").Height = Me.Controls("mpPhys").Pages(0).Controls("Frame8").InsideHeight - Me.Controls("mpPhys").Pages(0).Controls("Frame8").Controls("mpROM").Top
   ...
   '--- hook dynamic header button ---
On Error Resume Next
Set mHdrArchiveHook = New clsHdrBtnHook
Set mHdrArchiveHook.btn = Me.Controls("frHeader").Controls("cmdArchiveDelete")
On Error GoTo 0
...
RecalcBI
  ...
  Dim fBasic As MSForms.Frame: Set fBasic = CreateFrameP(hostBasic, "基本情報", 360)
    ...
    Set frAssist = BuildCheckFrame(fBasic, "補助具", COL_LX, Y, 370, MakeList(ASSISTIVE_CSV), "AssistiveGroup")
